...
// NOTE: If you want to share I2S BCLK/LRCK between an I2S mic + I2S amp/DAC,
// you must use a *single* I2S port (Duplex) so there is only one clock master.
// In Duplex mode, RX/TX share the same clock domain, so input/output sample
// rates must match.
...
// Duplex I2S: share BCLK/LRCK for both MIC (RX) and SPK (TX), keep separate
// data.
// - BCLK/LRCK: from the ESP32-S3 to both modules
// - DOUT: ESP32-S3 -> MAX98357 DIN
// - DIN:  INMP441 DOUT -> ESP32-S3
...
// Servo motor GPIO pins (adjust based on your hardware)
// Set to GPIO_NUM_NC if not used
#define SERVO1_GPIO GPIO_NUM_1
#define SERVO2_GPIO GPIO_NUM_2
