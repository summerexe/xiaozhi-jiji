...
void LcdDisplay::SetChatMessage(const char *role, const char *content) {
  ...
  // Collapse system messages (if it's a system message, check if the last
  // message is also a system message)
  ...
  if (strcmp(role, "user") == 0) {
    ...
    // Create full-width container for system messages to ensure center
    // alignment
    lv_obj_t *container = lv_obj_create(content_);
    lv_obj_set_width(container, LV_HOR_RES);
    lv_obj_set_height(container, LV_SIZE_CONTENT);
...
void LcdDisplay::SetupUI() {
  ...
  /* Top layer: Bottom bar - fixed at bottom, minimum height 48, height can be
   * adaptive */
  ...
  lv_label_set_long_mode(chat_message_label_,
                         LV_LABEL_LONG_WRAP); // Auto wrap mode
...
void LcdDisplay::SetEmotion(const char *emotion) {
  ...
  if (jiji_face_ != nullptr) {
    ...
    // Hide GIF/image/label, show JijiFace
    if (emoji_image_)
      lv_obj_add_flag(emoji_image_, LV_OBJ_FLAG_HIDDEN);
    if (emoji_label_)
      lv_obj_add_flag(emoji_label_, LV_OBJ_FLAG_HIDDEN);
    ...
    // Map Xiaozhi emotion names to JijiFace.
    auto is = [&](const char *name) { return strcmp(emotion, name) == 0; };

    // Reset transient effects every time so moods don't "leak" into each other.
    ...
    if (is("neutral") || is("idle")) {
      ...
      jiji_face_->anim_laugh();
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(true);
    } else if (is("laughing")) {
      jiji_face_->setMood(HAPPY);
      jiji_face_->anim_laugh();
      jiji_face_->anim_confused();
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(true);
    } else if (is("funny")) {
      jiji_face_->setMood(HAPPY);
      jiji_face_->anim_laugh();
      jiji_face_->anim_confused(); // extra wobble for "funny"
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(true);
    } else if (is("sad")) {
      jiji_face_->setMood(TIRED);
      jiji_face_->setPosition(S);
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(false);
    } else if (is("angry")) {
      jiji_face_->setMood(ANGRY);
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(false);
    } else if (is("crying")) {
      jiji_face_->setMood(TIRED);
      jiji_face_->setPosition(S);
      jiji_face_->setSweat(true); // closest available effect
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(false);
    } else if (is("loving")) {
      jiji_face_->setMood(HAPPY);
      jiji_face_->setAutoblinker(true, 2, 1);
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(false);
    } else if (is("embarrassed")) {
      jiji_face_->setMood(DEFAULT);
      jiji_face_->setSweat(true);
      jiji_face_->anim_confused();
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(false);
    } else if (is("surprised")) {
      jiji_face_->setMood(DEFAULT);
      jiji_face_->setPosition(N);
      jiji_face_->setAutoblinker(false);
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(false);
    } else if (is("shocked")) {
      jiji_face_->setMood(DEFAULT);
      ...
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(false);
      jiji_face_->blink(true, false);
    } else if (is("cool")) {
      jiji_face_->setMood(DEFAULT);
      jiji_face_->setPosition(E);
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(false);
    } else if (is("relaxed")) {
      jiji_face_->setMood(TIRED);
      jiji_face_->setAutoblinker(true, 4, 2);
      jiji_face_->setIdleMode(true, 8, 3);
      jiji_face_->setCuriosity(false);
    } else if (is("delicious")) {
      jiji_face_->setMood(HAPPY);
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(false);
    } else if (is("kissy")) {
      ...
      jiji_face_->setCuriosity(false);
      jiji_face_->blink(true, false);
    } else if (is("confident")) {
      jiji_face_->setMood(DEFAULT);
      jiji_face_->setPosition(NE);
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(false);
    } else if (is("sleepy")) {
      jiji_face_->setMood(TIRED);
      ...
      // Some integrations still send "tired"
      jiji_face_->setMood(TIRED);
      jiji_face_->setIdleMode(false);
      jiji_face_->setCuriosity(false);
    } else {
